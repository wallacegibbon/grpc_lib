...
write_server(Module) ->
    ...
    ok = file:write_file(ModuleName ++ ".erl",
                         [Header, Exports, Messages, Decoder, Services]).
...
write_client(Module) ->
    ...
    ok = file:write_file(ModuleName ++ ".erl",
                         [Header, Exports, Messages, TypeExports, Decoder,
                          Services]).
...
print_decoder(Module) ->
    to_io_list(["-spec decoder() -> module().\n",
                "%% The module (generated by gpb) used to encode and decode\n"
                "%% protobuf messages.\n",
                "decoder() -> ", Module, ".\n\n"]).
...
print_rpcs(RPCs, Service, client, Module) ->
    [print_rpc_call(R, Service, Module)
     || #{input_stream := false, output_stream := false} = R <- RPCs].
...
print_unary_rpc(#{name := Name, output := Output} = Rpc) ->
    [print_spec_call(Rpc),
...
print_client_to_server_streaming_rpc(#{name := Name, 
    ...
    ["-spec ", Name, 
     ...
     "    {", Output, "(), grpc:stream()} |\n",
     "    grpc:error_response().\n",
     "%% This is a client-to-server streaming RPC. "
     "After the client has sent the last message\n",
     "%% this function will be called a final time with 'eof' "
     "as the first argument. This last\n",
...
print_bidirectional_streaming_rpc(#{name := Name, 
    ...
    ["-spec ", Name, 
     ...
     "    {[", Output, "()], grpc:stream()} |\n",
     "    grpc:error_response().\n",
     "%% This is a bidirectional streaming RPC. If the client "
...
print_rpc_call(#{name := Name,  input := Input, output := Output} = _Rpc,
               Service,
               Module) ->
    ["-spec ", Name, "(\n",
