...
-spec auth_fun(Directory ::
                   string()) -> fun((cert()) -> {true, string()} | false).
...
-spec decode_input(ServiceName :: atom(),
                   RpcName :: atom(), DecoderModule :: module(),
                   Message :: binary() | eof) -> map() | eof.

%% @doc Decode input protobuf message to map.
decode_input(_, _, _, eof) -> eof;
decode_input(ServiceName, RpcName, DecoderModule,
             Msg) ->
    #{input := MsgName} =
        DecoderModule:fetch_rpc_def(ServiceName, RpcName),
    DecoderModule:decode_msg(Msg, MsgName).

-spec encode_output(ServiceName :: atom(),
                    RpcName :: atom(), DecoderModule :: module(),
                    Message :: map()) -> binary().

%% @doc Encode response message (map) to binary protobuf message.
encode_output(ServiceName, RpcName, DecoderModule,
              Msg) ->
    #{output := MsgName} =
        DecoderModule:fetch_rpc_def(ServiceName, RpcName),
    DecoderModule:encode_msg(Msg, MsgName).

-spec maybe_encode_header(Header ::
                              {grpc:metadata_key(),
                               grpc:metadata_value()}) -> {grpc:metadata_key(),
                                                           grpc:metadata_value()}.
...
-spec maybe_decode_header(Header ::
                              {grpc:metadata_key(),
                               grpc:metadata_value()}) -> {grpc:metadata_key(),
                                                           grpc:metadata_value()}.
...
decode(Base64) when byte_size(Base64) rem 4 == 2 ->
    base64:decode(<<Base64/bytes, "==">>);
decode(Base64) -> base64:decode(Base64).

-spec
     maybe_encode_headers(grpc:meta_data()) -> grpc:meta_data().
...
maybe_encode_headers(Headers) ->
    maps:map(fun (K, V) ->
                     case is_bin_header(K) of
                         true -> base64:encode(V);
                         false -> V
                     end
             end,
             Headers).

-spec keytake(Key :: term(),
              KVList :: [{term(), term()}],
...
keytake(Key, KVList, Default) ->
    case lists:keytake(Key, 1, KVList) of
        {value, {_, Value}, List2} -> {Value, List2};
        false -> {Default, KVList}
    end.
...
issuer_ids_from_directory(Dir) ->
    ...
    Keyfiles = lists:filter(fun (N) ->
                            ...
                            Filenames),
    maps:from_list([issuer_id_from_file(filename:join([Dir,
                                                       F]))
                    || F <- Keyfiles]).
