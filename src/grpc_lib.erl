...
-export([auth_fun/1, decode_input/4, encode_output/4, maybe_encode_header/1,
         maybe_encode_headers/1, maybe_decode_header/1, keytake/3]).
...
issuer_ids_from_directory(Dir) ->
    ...
    Keyfiles = lists:filter(fun is_keyfile/1, Filenames),
    maps:from_list([issuer_id_from_file(filename:join([Dir, F]))
                    || F <- Keyfiles]).

is_keyfile(Filename) ->
    Exts = [".pem", ".crt"],
    lists:any(fun (E) -> filename:extension(Filename) =:= E end, Exts).

-ifdef(EUNIT).

is_keyfile_test() ->
    ?assert(is_keyfile("abc.pem")),
    ?assert(is_keyfile("abc.crt")),
    ?assertNot(is_keyfile("abc.erl")).
